...
async def create_post(
    ...
    """통합 소식 작성 - 로컬 이미지 업로드 + 텍스트 선택"""
    logger.info(f"통합 소식 작성 요청: user_id={current_user.id}, has_content={bool(content)}, image_count={len(files)}")
    ...
    try:
        ...
        logger.info(f"통합 소식 작성 완료: post_id={new_post.id}, has_content={bool(new_post.content)}, image_count={len(image_urls)}")
        return PostResponse(**post_response_data)

    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"통합 소식 작성 중 오류: {str(e)}")
        await db.rollback()
        raise HTTPException(
...
@router.post("/with-images", response_model=PostResponse, deprecated=True)
async def create_post_with_images(
    post_data: PostCreateWithImages,
    current_user: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_db)
):
    """⚠️ DEPRECATED: 이미지와 함께 소식 작성 - 메인 POST / 엔드포인트 사용 권장
    
    새로운 통합 방식: POST /posts/ (파일 업로드 + content)
    - 로컬 이미지 파일을 직접 업로드
    - 자동 blob storage 처리
    - PDF 생성과 완전 호환"""
