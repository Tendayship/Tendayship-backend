...
async def setup_family_group(
    ...
    try:
        ...
        # 4. 그룹 생성 (내부에서 flush/refresh 처리됨)
        ...
        issue_data = {
            ...
            "status": IssueStatus.OPEN  # Enum 사용 (CRUD에서 .value로 처리 가정)
        ...
        logger.exception(f"가족 그룹 설정 중 오류가 발생했습니다: {str(e)}")  # 상세 예외 로깅 추가
...
async def create_family_group(
    ...
    try:
        ...
        logger.exception(f"가족 그룹 생성 중 오류가 발생했습니다: {str(e)}")  # 상세 예외 로깅 추가
...
async def delete_my_family_group(
    ...
    try:
        ...
        logger.exception(f"그룹 삭제 중 오류: {str(e)}")  # 상세 예외 로깅 추가
